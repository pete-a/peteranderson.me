...
export const ToggleSwitch = (props: { checked: boolean, theme: Theme, onToggle: () => any }) => {
    ...
    return (
        <button className={`${themeStyles.container} ${sharedStyles.container}`} onClick={props.onToggle}>
            <div className={`${sharedStyles.toggle} ${themeStyles.toggleTheme} ${checkedStyle}`}></div>
        </button>
    );
}
